...
public:
    ...
    explicit Logger(EnginePtr engine)
        : engine_(std::move(engine)), runtimeLevel_(LogLevel::Info), matlabEnabled_(true),
          fileEnabled_(false), buffered_(false) {}
    ...
    template<LogLevel Level> void log(const std::string& msg) {
    ...
    // Convenience methods
    template<typename... Args> void trace(const std::string& msg, const Args&... args) {
        log<LogLevel::Trace>(msg, args...);
    }

    template<typename... Args> void debug(const std::string& msg, const Args&... args) {
        log<LogLevel::Debug>(msg, args...);
    }

    template<typename... Args> void info(const std::string& msg, const Args&... args) {
        log<LogLevel::Info>(msg, args...);
    }

    template<typename... Args> void warn(const std::string& msg, const Args&... args) {
        log<LogLevel::Warn>(msg, args...);
    }

    template<typename... Args> void error(const std::string& msg, const Args&... args) {
        log<LogLevel::Error>(msg, args...);
    }

    template<typename... Args> void fatal(const std::string& msg, const Args&... args) {
...
private:
    std::string formatMessage(LogLevel level, const std::string& msg) {
        ...
        oss << std::put_time(std::localtime(&time), "%H:%M:%S") << '.' << std::setfill('0')
            << std::setw(3) << ms.count() << " [" << log_level_str(level) << "] " << msg;
        return oss.str();
    }
    ...
    void writeToMatlab(const std::string& msg) {
        ...
        try {
            ...
            engine_->feval(u"fprintf", {factory.createScalar(msg + "\n")}, {});
        } catch (...) {
