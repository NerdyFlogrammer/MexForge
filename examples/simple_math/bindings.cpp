...
public:
        ...
        mexforge::Logger& logger) override
    {
        ...
        logger.debug("ApplyRunner: applying '", func,
                     "' to ", std::to_string(data.size()), " elements");

        auto result = calc.apply(data, func);

        outputs[0] = factory.createArray(
            {1, result.size()}, result.begin(), result.end());
    }
};
    ...
    void setup(mexforge::RegistryBuilder<Calculator>& b) override {
        ...
        b.bind_free_lambda<std::string>("create",
         ...
         .bind_free_lambda<uint32_t>("destroy",
            [this](uint32_t id) {
                store().remove(static_cast<int>(id));
            })
        ...
        // --- Tier 2: Lambda (custom logic, auto-marshalling) ---
         .bind_lambda<double, double, std::string>("compute",
            [](Calculator& calc, double a, double b, std::string op) -> double {
                return calc.compute(a, b, op);
            })

         .bind_lambda<double, double, int32_t>("linspace",
            [](Calculator& calc, double start, double stop, int32_t n)
                -> std::vector<double> {
